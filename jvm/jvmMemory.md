...
## 1. java内存组成介绍：堆（Heap）和非堆（Non-heap）
官方解释：“Java虚拟机具有在所有Java虚拟机线程中共享的堆。堆是所有类实例和数组的内存分配的运行时数据区”，“在JVM中堆之外的内存称为非堆内存(Non-heap memory)”。
...
由于现在收集器都是采用分代收集算法，堆被划分为新生代和老年代。新生代主要存储新创建的对象和尚未进入老年代的对象。老年代存储经过多次新生代GC(Minor GC)任然存活的对象。<br>
